...
class PhysicsSimV1():
    ...
    def get_moments(self, thrusts):
        ...
        # self.T_q = 0.1
        # thrust_moment = np.array([
        #     (thrusts[0] + thrusts[3] - thrusts[1] - thrusts[2]) * self.l_to_rotor,
    ...
    def next_timestep(self, rotor_speeds):
        ...
        # TODO: should count how many rounds the copter has turned
...
#                 print('done, less than lower bound', ii, position[ii], self.lower_bounds[ii])
                new_positions.append(self.lower_bounds[ii])
                self.done = True
            elif position[ii] > self.upper_bounds[ii]:
#                 print('done, larger than upper bound', ii, position[ii], self.upper_bounds[ii])     
...
#             print('done, larger than runtime', self.time, self.runtime)               
            self.done = True
        return self.done